...
void loop() {
    ...
    Serial.println(String(energy));
    if( !isnan(energy)&& temp != energy ){
        Firebase.setFloat("/sems006/unit", energy);
        String days = Firebase.getString("/sems006/hours");
         dtostrf(energy,10, 3, outstr);
         String data = outstr;
         data.replace(" ","");
        String x = days+","+data;
        Serial.println(x);
        Firebase.setString("/sems006/hours",x);
        temp = energy;
    } else {
        Serial.println("Error reading energy/not yet changed");
    }
