...
class Settings extends Component {
  ...
  componentDidMount() {
    ...
    const readUsersData = () => {
      firebase
        .database()
        .ref(meterId.toString() + "/unit") // setting meter id and database address
        .on("value", (snapshot) => {
          if (this._isMounted) {
            ...
            switch (true) {
              case unit_value <= 50:
                unit_cost = 2.0;
                Electricity_cost = unit_value * 2;
                break;
              case unit_value > 50 && unit_value <= 100:
                unit_cost = 5.79;
                Electricity_cost = unit_value * 5.79;
                break;
              case unit_value > 100 && unit_value <= 200:
                unit_cost = 8.11;
                sub_total = 100 * 5.79;
                Electricity_cost = (unit_value - 100) * 8.11 + sub_total;
                break;
              case unit_value > 200 && unit_value <= 300:
                unit_cost = 10.2;
                sub_total = 200 * 8.11;
                Electricity_cost = (unit_value - 200) * 10.2 + sub_total;
                break;
              case unit_value > 300 && unit_value <= 700:
                unit_cost = 17.6;
                sub_total = 300 * 10.2;
                Electricity_cost = (unit_value - 300) * 17.6 + sub_total;
                break;
              case unit_value > 700:
                unit_cost = 20.7;
                sub_total = 700 * 17.6;
                Electricity_cost = (unit_value - 700) * 20.7 + sub_total;
                break;
            }
